...
class ImportSpongebobCards extends Command
{
    protected $signature = 'import:spongebob-cards 
                            {--file= : Path al file CSV}
                            {--limit= : Limite di righe da processare}
                            {--chunk=1000 : Dimensione del chunk}';

    protected $description = 'Importa le carte di Spongebob dal file CSV';
    ...
    public function handle()
    {
        ...
        $this->info("🚀 Inizio importazione carte Spongebob...");

        $category = Category::where('slug', 'spongebob')->first();
        if (!$category) {
            $category = Category::create([
                'name' => 'Spongebob',
                'slug' => 'spongebob',
                'description' => 'Carte da collezione di Spongebob',
                'is_active' => true,
                'sort_order' => 5,
            ]);
            $this->info("✅ Creata categoria Spongebob");
        }
        ...
        try {
            foreach ($chunk as $row) {
                try {
                    ...
                    \Log::error("ImportSpongebobCards - Errore riga", [
                        'name' => $cardName,
                        'number' => $cardNumber,
                        'error' => $e->getMessage(),
